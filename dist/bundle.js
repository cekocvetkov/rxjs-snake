...
document.addEventListener("DOMContentLoaded", function () {
    const keydown$ = (0, rxjs_1.fromEvent)(window, "keydown");
    const refresh$ = (0, rxjs_1.fromEvent)(document.getElementById("refresh"), "click");
    refresh$.subscribe((e) => refreshGame());
    keydown$
        .pipe((0, rxjs_1.map)((event) => DIRECTIONS[event.key]), (0, rxjs_1.filter)((direction) => !!direction), 
    // startWith("RIGHT"),
    (0, rxjs_1.tap)((direction) => (DIRECTION = direction)), (0, rxjs_1.tap)((dir) => console.log(dir)), (0, rxjs_1.scan)(nextDirection), (0, rxjs_1.distinctUntilChanged)())
        .subscribe();
    main();
});
...
function main() {
    ...
    if (gameOver()) {
        canvas_1.ctx.fillStyle = "pink";
        canvas_1.ctx.fillRect(0, 0, canvas_1.canvas.width, canvas_1.canvas.height);
        canvas_1.ctx.fillStyle = "red";
        canvas_1.ctx.font = "48px serif";
        const text = canvas_1.ctx.measureText("Game Over"); // TextMetrics object
        canvas_1.ctx.fillText("Game Over", canvas_1.canvas.width - canvas_1.canvas.width / 2 - text.width / 2, canvas_1.canvas.height - canvas_1.canvas.height / 2);
        return;
    }
    moveSnake();
    drawSnake();
...
function drawSnake() {
    for (const s of snake) {
        canvas_1.ctx.fillStyle = "red";
        canvas_1.ctx.fillRect(s.x, s.y, SEGMENT_SIZE, SEGMENT_SIZE);
    }
}
function moveSnake() {
    if (!DIRECTION) {
...
function gameOver() {
    ...
    if (snakeHead.x < 0 || snakeHead.x > canvas_1.canvas.width || snakeHead.y < 0 || snakeHead.y > canvas_1.canvas.height) {
        return true;
    }
}
